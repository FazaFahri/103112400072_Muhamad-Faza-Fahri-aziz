...
## Dasar Teori
Doubly Linked List merupakan salah satu struktur data linear dinamis yang terdiri dari sekumpulan node, di mana setiap node memiliki tiga komponen utama, yaitu data, pointer ke node sebelumnya (prev), dan pointer ke node berikutnya (next). Berbeda dengan singly linked list yang hanya dapat ditelusuri satu arah, doubly linked list memungkinkan penelusuran data secara dua arah, baik dari awal ke akhir maupun sebaliknya. Struktur ini memberikan fleksibilitas yang lebih tinggi dalam operasi penyisipan dan penghapusan node, terutama ketika operasi dilakukan di tengah list, karena akses ke node sebelumnya tidak memerlukan proses penelusuran ulang dari awal. Namun, kelebihan tersebut diimbangi dengan kebutuhan memori yang lebih besar akibat penggunaan dua pointer pada setiap node. Doubly linked list banyak digunakan pada aplikasi yang memerlukan navigasi dua arah, seperti implementasi undo–redo, manajemen cache, dan sistem antrian kompleks.[1]
...
```
Program C++ ini menerapkan struktur data **Doubly Linked List** untuk menyimpan informasi kendaraan yang meliputi nomor polisi, warna, dan tahun pembuatan. Melalui implementasi tersebut, program memperlihatkan proses pencarian data serta penghapusan elemen dalam struktur doubly linked list secara terstruktur.
...
## Kesimpulan
program Doubly Linked List ini mampu mengelola data kendaraan secara dinamis melalui operasi penambahan, pencarian, dan penghapusan data. Penggunaan dua pointer pada setiap node memungkinkan proses penghapusan dan penelusuran data dilakukan dengan lebih fleksibel dibandingkan singly linked list. Program ini memberikan pemahaman praktis mengenai cara kerja doubly linked list serta pemanfaatannya dalam pengelolaan data yang membutuhkan akses dua arah secara efisien.



## Referensi 
[1].Hwang, K., & Briggs, F. A. (1984). Computer Architecture and Parallel Processing. IEEE Transactions on Computers, 33(10), 987–1000.
